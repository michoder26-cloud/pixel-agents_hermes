...
async function main(): Promise<void> {
  ...
  // copyHookScript() expects the extension ROOT and appends dist/hooks/ itself
  // (the VS Code adapter passes context.extensionPath). The bundled CLI runs from
  // inside dist/, so step up one level — otherwise the source path doubles to
  // dist/dist/hooks/ and the hook script is never installed.
  ...
  try {
    ...
    // Hermes integration: push-based provider with its own ingestion bridge.
    // Shares the SAME store as the Claude runtime so both render in one office.
    const hermesBridge = new HermesBridge(store, hermesProvider);

    // Wire hook events: HTTP POST -> route by providerId -> agents
    server.onHookEvent((providerId, event) => {
      if (providerId === 'hermes') {
        hermesBridge.handleEvent(event);
      } else {

...
import { claudeProvider, hermesProvider } from './providers/index.js';
...
function handleWebviewReady(send: WsSend, ctx: ClientMessageContext): void {
  ...
  // Union across all active providers — the webview keeps a single global set,
  // so Hermes' reading tools must be merged in for correct read/typing animations.
  send({
    type: 'providerCapabilities',
    readingTools: [...claudeProvider.readingTools, ...hermesProvider.readingTools],
    subagentToolNames: [
      ...claudeProvider.subagentToolNames,
      ...hermesProvider.subagentToolNames,
    ],
  });

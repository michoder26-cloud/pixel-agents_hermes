...
export function startStaleExternalAgentCheck(
  ...
  return setInterval(() => {
    ...
    for (const [id, agent] of agents) {
      if (!agent.isExternal) continue;
      // Skip non-Claude providers (e.g. Hermes): they have no JSONL file and are
      // owned by their own bridge, not this scanner. Statting their empty
      // jsonlFile would throw and wrongly despawn them.
